...
def get_whole_val_set(input_transform, img_size):
    ...
    return WholeDatasetFromStruct(
        structFile, input_transform=input_transform, img_size=img_size
    )
...
def get_whole_training_set(input_transform, img_size, onlyDB=False):
    ...
    return WholeDatasetFromStruct(
        structFile, input_transform=input_transform, onlyDB=onlyDB, img_size=img_size
    )
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(
        self, structFile, input_transform=None, onlyDB=False, img_size=(480, 640)
    ):
        super().__init__()
        ...
        self.images_path = [
            join_db_img(root_dir, dbIm) for dbIm in self.dbStruct.dbImage
        ]
        self.images_name = [dbIm for dbIm in self.dbStruct.dbImage]
        self.poses = self.read_poses("db", dataset_name)
        if not onlyDB:
            self.images_path += [join(queries_dir, qIm) for qIm in self.dbStruct.qImage]
            self.images_name += [qIm for qIm in self.dbStruct.qImage]
            self.poses = {**self.poses, **self.read_poses("query", dataset_name)}
